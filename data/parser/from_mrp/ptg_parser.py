...
class PTGParser(AbstractParser):
    def __init__(self, args, framework: str, language: str, part: str, fields, precomputed_dataset=None, filter_pred=None, **kwargs):
        ...
        utils.create_edge_permutations(self.data, PTGParser.node_similarity_key)
        ...
        super(PTGParser, self).__init__(fields, self.data, filter_pred)
    ...
    def initialize(self, args, raw_path, cache_path, companion_path, precomputed_dataset=None):
        ...
        if precomputed_dataset is None:
            utils.create_possible_rules(data, PTGParser._create_possible_rules, prune=True)
            rule_set = utils.get_smallest_rule_set(data, approximate=self.language == "eng")
        else:
            utils.create_possible_rules(data, PTGParser._create_possible_rules, prune=False)
